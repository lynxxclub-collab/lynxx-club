-- Add authorization check to send_message function
-- This ensures the sender_id matches the authenticated user
CREATE OR REPLACE FUNCTION public.send_message(p_sender_id uuid, p_recipient_id uuid, p_conversation_id uuid, p_content text, p_message_type text DEFAULT 'text'::text)
 RETURNS json
 LANGUAGE plpgsql
 SECURITY DEFINER
...
-- Add authorization check to charge_video_date_transaction function
-- This ensures the caller is part of the video date
CREATE OR REPLACE FUNCTION public.charge_video_date_transaction(p_video_date_id uuid, p_seeker_id uuid, p_earner_id uuid, p_credits_charged integer, p_earner_amount numeric, p_platform_fee numeric, p_usd_amount numeric)
 RETURNS json
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
