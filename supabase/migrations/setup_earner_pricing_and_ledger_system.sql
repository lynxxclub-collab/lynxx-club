-- ============================================
-- MIGRATION: setup_earner_pricing_and_ledger_system.sql
-- DESCRIPTION: Sets up roles, pricing constraints, 
--              date management, and financial ledgers for 
--              Seekers and Earners.
-- ============================================
...
-- 2. Extend the auth.users table with a public profile
...
-- 3. Wallets Table to track user balances
...
-- 4. Earner Service Pricing Table
...
-- 6. Transaction Ledger (Full Accounting)
...
-- 7. Function to Handle Payouts when a Date is 'completed'
...
BEGIN
    ...
    IF NEW.status = 'completed' AND OLD.status != 'completed' THEN
        ...
        -- You might want a separate platform wallet table, or just rely on this ledger.
        INSERT INTO public.ledger (date_id, from_user_id, to_user_id, amount_credits, transaction_type, description)
        VALUES (
            NEW.id, 
            NEW.seeker_id, 
...
-- 8. Trigger to connect the function to the dates table
...
-- Seekers should NOT see breakdowns, only their own booking details
