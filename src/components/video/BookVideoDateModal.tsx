...
export default function BookVideoDateModal({
  ...
  const [existingDates, setExistingDates] = useState<{ start: Date; duration: number }[]>([]);
  ...
  // Update current time every minute to keep time slots fresh
  useEffect(() => {
    if (!open) return;
  ...
  const isSlotBooked = (slotDate: Date) => {
    const slotEnd = addMinutes(slotDate, parseInt(duration));

    for (const existing of existingDates) {
      const existingEnd = addMinutes(existing.start, existing.duration);

      // Check for overlap
      if (
        (slotDate >= existing.start && slotDate < existingEnd) ||
        (slotEnd > existing.start && slotEnd <= existingEnd) ||
        (slotDate <= existing.start && slotEnd >= existingEnd)
      ) {
        return true;
      }
    }
  ...
  const getAvailableTimeSlots = () => {
    ...
    dayAvailability.forEach((avail) => {
      ...
      const slotDate = setMinutes(setHours(selectedDate, startH), startM);
    ...
    // Remove duplicates
    const uniqueSlots = slots.filter((slot, index, self) => index === self.findIndex((s) => s.value === slot.value));

    return uniqueSlots;
  };

  const timeSlots = getAvailableTimeSlots();
  const availableTimeSlots = timeSlots.filter((slot) => !slot.disabled);

  // Check if a date has any available slots
  const dateHasAvailability = (date: Date) => {
    if (earnerAvailability.length === 0) return true;
  ...
  const fetchExistingBookings = async () => {
    const { data: bookings } = await supabase
      ...
      .in("status", ["scheduled", "pending", "waiting", "in_progress"])
      .gte("scheduled_start", new Date().toISOString());

    if (bookings) {
      setExistingDates(
        bookings.map((d) => ({
          start: new Date(d.scheduled_start),
          duration: d.scheduled_duration,
        })),
      );
    }
  };
  ...
  useEffect(() => {
    ...
    if (open) {
      fetchData();
      setCurrentTime(new Date()); // Refresh current time when modal opens
    }
  }, [open, earnerId]);
  ...
  // Real-time subscription for video_dates changes (new bookings)
  useEffect(() => {
    if (!open || !earnerId) return;

    const channel = supabase
      .channel(`earner-bookings-${earnerId}`)
      .on(
        "postgres_changes",
        {
          event: "*",
        ...
        () => {
          ...
          if (selectedTime && selectedDate) {
            ...
            if (isSlotBooked(slotDate)) {
              setSelectedTime("");
              toast.warning("Selected time slot is no longer available");
            }
          }
        },
      )
  ...
  }, [open, earnerId, selectedTime, selectedDate]);
  ...
  const validateBooking = (): string | null => {
    ...
    // Only check if the time is in the past (no minimum advance time required)
    ...
    // Check for conflicts
    if (isSlotBooked(scheduledStart)) {
          ...
          <div className="space-y-5 py-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                >
                  ...
                  <div className="text-left">
                    <p className={cn("font-medium text-sm", callType === "video" ? "text-white" : "text-white/70")}>
                      Video
                    </p>
                ...
                >
                  ...
                  <div className="text-left">
                    <p className={cn("font-medium text-sm", callType === "audio" ? "text-white" : "text-white/70")}>
                      Audio
                    </p>
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                >
                  ...
                  >
                    ...
                    <SelectValue placeholder={availableTimeSlots.length === 0 ? "No slots" : "Time"} />
                  </SelectTrigger>
                  <SelectContent className="bg-[#0a0a0f] border-white/10 max-h-60">
                    {timeSlots.length === 0 ? (
                      <div className="px-2 py-1.5 text-sm text-white/50">No available times</div>
                    ) : (
                      timeSlots.map((slot) => (
                        <SelectItem
                          key={slot.value}
                          ...
                          className={cn(
                            ...
                            slot.disabled && "text-white/30 line-through",
                          )}
                        >
                          {slot.label}
            ...
            <div className="p-4 rounded-xl bg-white/[0.02] border border-white/10 space-y-3">
              ...
              <div className="flex items-center justify-between text-sm">
                ...
                <span className={cn("font-semibold", hasEnoughCredits ? "text-white" : "text-destructive")}>
            ...
            <div className="flex items-start gap-3 p-3 rounded-xl bg-amber-500/10 border border-amber-500/20 text-sm">
              ...
              <p className="text-white/70">
                Credits will be reserved until the call ends. They will only be charged after successful completion.
              </p>
            </div>
          </div>
